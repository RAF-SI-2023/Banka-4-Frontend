...
const ResetPasswordPage = () => {
  ...
  const sendRequest = async () => {
    try {
      await makeApiRequest(
        UserRoutes.user_generate_reset,
        "POST",
        { email },
        true,
        true,
      ...
      if (ctx) {
        ctx.setErrors([
          ...ctx.errors,
          "Our Success: Kod je uspesno poslat na mejl",
        ]);
      }
    } catch (error) {}
  };
  ...
  const resetPassword = async () => {
    try {
      await makeApiRequest(
        UserRoutes.user_reset_password,
        "POST",
        { email, sifra: newPassword, kod: activationCode },
        true,
        true,
        ctx
      );
      if (ctx) {
        ctx.setErrors([
          ...ctx.errors,
          "Our Success: Lozinka je uspesno resetovana",
        ]);
      }
    } catch (error) {}
  };
