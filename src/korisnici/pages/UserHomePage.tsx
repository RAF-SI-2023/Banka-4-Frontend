...
const UserHomePage: React.FC = () => {
  ...
  const fetchAccounts = async () => {
    try {
      ...
      const marzniData = await makeGetRequest(`/marzniRacuni/${me.id}`);
      if (marzniData) {
        setMarzniRacuni(marzniData);
        console.log(marzniData);
      }
    } catch (error) {
      console.error(error);
    }
  ...
  return (
    <PageWrapper>
      <HeadingText>Lista Racuna</HeadingText>
      <FormWrapper>
        <ScrollContainer>
              ...
              {accounts?.map((account, index) => (
                <TableRow key={account.brojRacuna}>
                  <HighlightableStyledTableCentered
                    className={"idRacunaTd" + index}
                    id={account.brojRacuna}
                    component="th"
                    scope="row"
          ...
          <Table aria-label="marzni racuni table">
            <TableHead>
              <TableRow>
                <StyledTableCentered>Broj Marznog racuna</StyledTableCentered>
